...
def main():
    ...
    filter_result = filter_result.where(merge_result["model_signal"] > 0).dropna()
    filter_result = filter_result.where(merge_result["profit_risk_ratio"] > 2.0).dropna()
    filter_result = filter_result.where(merge_result["profit_risk_ratio"] < 10.0).dropna()
    ...
    filter_result = filter_result.ix[:, ["day_date", "deviation_signal", "model_signal", "day_close", "enter_point", "stop_point", "target_point", "profit", "risk", "profit_risk_ratio", "profit_ratio", "day_atr1_high", "day_atr1_low", "day_win_percentage", "stock_id"]]
